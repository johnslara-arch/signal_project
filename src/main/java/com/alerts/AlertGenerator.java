...
public class AlertGenerator {
     ...
     * method called by the checkers. This method should define the specific conditions under which an
    ...
    /**
     * Used to filter relevant records (e.g. ECG data) retrieved in the {@code DataStorage}. Called by the 
     * specified alert checker. Records are returned in the same order, insertion order, as they were in {@code records}.
     * 
    ...
    /**
     * Check blood pressure (BP) records for two alert generating conditions; consecutive trend and critical threshold. Consecutive trend consists of 3 consecutive
     * reading changing by more than 10 mmHg in the same direction (increasing or decreasing). Critical threshold is met when systolic BP falls below 90 mmHg or above 180 mmHg, 
    ...
    private void checkBpTrendAlert(List<PatientRecord> records, String label){
        ...
        for (int i = 2; i < records.size(); i++){
            ...
            if (increasing){
                triggerAlert(new Alert(String.valueOf(records.get(i).getPatientId()), label + " Blood Pressure shows increasing trend", records.get(i).getTimestamp()));
            } else if (decreasing){
                triggerAlert(new Alert(String.valueOf(records.get(i).getPatientId()), label + " Blood Pressure shows decreasing trend", records.get(i).getTimestamp()));
            }
        }
     ...
     * @param label type of BP that triggers alert.
     */
    private void checkBpCriticalThresholds(List<PatientRecord> records, double lowerBound, double higherBound, String label){
        for (PatientRecord record : records){
            double value = record.getMeasurementValue();
            if (value > higherBound){
                triggerAlert(new Alert(String.valueOf(record.getPatientId()), label + " Blood Pressure critically high: " + value, record.getTimestamp()));
            } else if (value < lowerBound){
                triggerAlert(new Alert(String.valueOf(record.getPatientId()), label + " Blood Pressure critically low: " + value, record.getTimestamp()));
            }
        }
    }
    ...
    /**
     * Checks blood saturation levels for low saturation (less than 92%) or rapid drops in saturation (more than or equal to 5% within 10 minutes window).
    ...
    private void checkBloodSaturationAlerts(List<PatientRecord> allRecords){
        ...
        for (int i = 0; i < saturationRecords.size(); i++){
            double value = saturationRecords.get(i).getMeasurementValue(); // In patient records saturation data is already stored as a double not a string such as 95%.

            if (value < 92){
                triggerAlert(new Alert(String.valueOf(saturationRecords.get(i).getPatientId()), "Low Blood Saturation detected: " + value + "%", saturationRecords.get(i).getTimestamp()));
            }
            ...
            for (int j = i - 1; j >= 0; j--){
                ...
                if (oldValue - value >= 5){
                    triggerAlert(new Alert(String.valueOf(saturationRecords.get(i).getPatientId()), "Rapid Blood Saturation drop from " + oldValue + "% to" + value, saturationRecords.get(i).getTimestamp()));
                    break;
                }
            }
        }
    ...
    /**
     * Checks for a combined alert condition. Triggers an alert if both systolic BP is below 90 mmHg
     * and blood saturation levels are below 92% at the same time (defined time proximity as records occuring within a minute of one another).
     * 
     * @param allRecords patient records retrieved from {@DataStorage}.
     */
    private void checkHypotensiveHypoxemiaAlert(List<PatientRecord> allRecords){
        ...
        for (PatientRecord systolicRecord : systolicRecords){
            ...
            for (int i = saturationRecords.size() - 1; i >= 0; i--){
                ...
                // Assume that if saturation records are taken over a minute before systolic BP record being examined they are not relevant.
                ...
                if (saturationRecord.getMeasurementValue() < 92){
                    triggerAlert(new Alert(String.valueOf(systolicRecord.getPatientId()), "Hypotensive Hypoxemia Alert!", systolicRecord.getTimestamp()));
                    break;
                }
            }
    ...
    /**
     * Checks ECG records for values that exceed a sliding window average using {@value #ECG_WINDOW_SIZE} by a factor of {@value #ECG_PEAK_MULTIPLIER}.
     * The size of the factor and number of values used to calculate sliding window have been assumed to be 2 and 20 respectively. Triggers an alert when
    ...
    private void checkEcgAlerts(List<PatientRecord> allRecords){
        ...
        for (int i = ECG_WINDOW_SIZE; i < ecgRecords.size(); i++){
            ...
            if (average > 0 && ecgRecords.get(i).getMeasurementValue() > ECG_PEAK_MULTIPLIER*average){
                triggerAlert(new Alert(String.valueOf(ecgRecords.get(i).getPatientId()), "Abnormal ECG peak detected: " + ecgRecords.get(i).getMeasurementValue(), ecgRecords.get(i).getTimestamp()));
            }
        }
    }
    ...
    /**
     * Checks records for manually activated alerts, which can be done by a nurse or a patient pressing the alert button.
     * If the "triggered" condition is found, a respective alert is triggered.
     * As defined in {@code DataReaderOutputFile}, a "triggered" condition is found when the data value is equal to 1.0.
     * 
     * @param allRecords patient records retrieved from {@DataStorage}.
     */
    private void checkTriggeredAlerts(List<PatientRecord> allRecords){
        ...
        for (PatientRecord record : alertRecords){
            if (record.getMeasurementValue() == 1.0){
                triggerAlert(new Alert(String.valueOf(record.getPatientId()), "Manual Alert triggered", record.getTimestamp()));
            }
        }
    }
    ...
    protected void triggerAlert(Alert alert) {
        System.out.printf("ALERT -> Patient ID: %s | Condition: %s | Time: %d%n", alert.getPatientId(), alert.getCondition(), alert.getTimestamp());
    }
}
