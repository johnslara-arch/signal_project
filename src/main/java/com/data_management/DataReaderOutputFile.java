...
/**
 * Implements {@link DataReader} to fetch output files produced by {@code FileOutputStrategy} and read their data.
 * The data held in each file is patient records of the form:
 *      Patient ID: 1, Timestamp: ..., Label: HeartRate, Data: ...
 * After reading every .txt file in the specified directory, it stores the data in {@link DataStorage}.
 */
public class DataReaderOutputFile implements DataReader {
    ...
    /**
     * Goes through all the text files in a valid directory and stores valid records into {@code dataStorage}.
     * 
    ...
    /**
     * Parses one file at a time and adds each valid record to {@code dataStorage} once parsed.
     * 
     ...
     * Splits parts of the String on ", " with a look ahead (e.g. for "Patient ID:") to ensure data
     * stays together.
     * 
     * @param line the line to parse through.
     * @param dataStorage the storage space to add the obtained data to.
     * 
     * @throws IllegalArgumentException if line has wrong format so returns more or less than 4 fields.
     */
    private void parseLine(String line, DataStorage dataStorage){

        String[] parts = line.split(", (?=[A-Z][a-z]+ [A-Z][a-z]+:)");
        if (parts.length != 4){
    ...
    /**
     * Extracts the actual value from the parts separated in the {@code parseLine} method.
     * 
     * @param part the string of the entire part parsed in parseLine.
     * @param typeOfData the type of value being extracted, used to indicate to user which part of the data throws the exception.
     * 
     * @throws IllegalArgumentException if the delimiter for the data, the colon, is missing.
     * 
    ...
    /**
     * Used to convert the string of the data value to a double. Multiple scenarios are possible.
     * For example, blood saturation values need to have "%" stripped.
     * An assumption was made to convert "triggered" values of alerts to 1.0 and "resolved" values to 0.0.
     * 
